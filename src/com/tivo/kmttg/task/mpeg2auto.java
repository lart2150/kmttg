...
public class mpeg2auto {
   private backgroundProcess process;
   private jobData job;

   // constructor
   public mpeg2auto(jobData job) {
      debug.print("job=" + job);
      this.job = job;
   }
   ...
   public Boolean launchJob() {
      ...
      if ( ! file.isFile(config.mpeg2auto) ) {
         log.error("mpeg2auto not found: " + config.mpeg2auto);
         schedule = false;
      }
            
      if ( ! file.isFile(job.tsFile) ) {
         log.error("ts file not found: " + job.tsFile);
         schedule = false;
      }
   ...
   private Boolean start() {
      ...
      command.add(config.mpeg2auto);
      command.add(job.tsFile);
      process = new backgroundProcess();
      log.print(">> Running mpeg2auto on " + job.tsFile + " ...");
      ...
      if (exit_code == -1) {
         ...
         if (config.GUI) {
            ...
            if ( jobMonitor.isFirstJobInMonitor(job) ) {
               ...
               String title = String.format("mpeg2auto: %s %s", t, config.kmttg);
        ...
        return true;
         ...
         if (failed == 1) {
            log.error("mpeg2auto failed (exit code: " + exit_code + " ) - check command: " + process.toString());
            process.printStderr();
         } else {
