...
read($s_stream, $ibuf, 732 + $hd) == 732 + $hd or die; # read Header Object and data header
$skip = 682 + $hd;
unpack("x$skip H32", $ibuf) eq '3626b2758e66cf11a6d900aa0062ce6c' or die;       # data hdr where expected
substr($ibuf, 0x1fa + $hd, 10) = pack("v x8", 0x9012); # fix audio header so VLC will play
...
while (read($s_stream, $ibuf, $pktsiz) == $pktsiz) { # read packet from stream
    ...
    for ($p = 0; $p < ($pcnt & 63); $off += $len, ++$p) { # check each payload
        ...
        next unless $sn == 0x82 || $sn == 1; # process only video key frame and audio
